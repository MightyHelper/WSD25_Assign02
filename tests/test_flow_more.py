...
def test_order_item_delete_and_pay_empty(test_app, admin_user: UserWithLogin, normal_user: UserWithLogin):
    user, user_headers = normal_user
    user_id = user.id
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "OrderDelBook", "author_id": None}, headers=admin_user[1])
    assert r.status_code == 201

    order_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/orders/", json={"id": order_id, "user_id": user_id}, headers=user_headers)
    assert r.status_code == 201
...
def test_comment_like_already_liked(test_app, admin_user: UserWithLogin, normal_user: UserWithLogin):
    user, user_headers = normal_user
    user_id = user.id
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "CLB", "author_id": None}, headers=admin_user[1])
    assert r.status_code == 201
    review_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/reviews/", json={"id": review_id, "book_id": book_id, "user_id": user_id, "title": "R", "content": "c"}, headers=user_headers)
    assert r.status_code == 201
...
def test_books_extra_cover_db_storage(monkeypatch, test_app, admin_user: UserWithLogin):
    # force DB storage
    monkeypatch.setattr(settings, "STORAGE_KIND", StorageKind.DB)
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "DBCover", "author_id": None}, headers=admin_user[1])
    assert r.status_code == 201
    data = b"myblob"
...
def test_security_invalid_scheme(test_app, admin_user: UserWithLogin, normal_user: UserWithLogin):
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "SecBook", "author_id": None}, headers=admin_user[1])
    assert r.status_code == 201
