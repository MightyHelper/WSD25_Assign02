...
def test_like_book_user_not_found(test_app, admin_user: UserWithLogin):
    # create a book but use a token whose subject doesn't map to an existing user
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "Edge Book", "author_id": None}, headers=admin_user[1])
...
def test_upload_cover_empty_body_returns_400(test_app, admin_user: UserWithLogin, normal_user: UserWithLogin):
    # create user and book via fixtures
...
def test_get_cover_path_missing_returns_404(test_app, admin_user: UserWithLogin, normal_user: UserWithLogin, tmp_path):
    # create user and book and set cover_path to missing file
    user, headers = normal_user
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "MissingPath", "author_id": None}, headers=admin_user[1])
    assert r.status_code == 201
...
def test_comment_like_unlike_flow(test_app, admin_user: UserWithLogin, normal_user: UserWithLogin):
    # create user via fixture
    user, headers = normal_user

    # create a book and then create review and comment via the new nested endpoint
    book_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/books/", json={"id": book_id, "title": "Rb", "author_id": None}, headers=admin_user[1])
    assert r.status_code == 201
    review_id = str(uuid.uuid4())
    r = test_app.post("/api/v1/reviews/", json={"id": review_id, "book_id": book_id, "user_id": user.id, "title": "R", "content": "c"}, headers=headers)
    assert r.status_code == 201
    comment_id = str(uuid.uuid4())
    r = test_app.post(f"/api/v1/reviews/{review_id}/comments", json={"id": comment_id, "user_id": user.id, "content": "hey"})
    assert r.status_code == 201
